...
export const reomveImageBackground = async (req, res) => {
  try {
    const { userId } = req.auth();
    ...
    const formData = new FormData();
    formData.append("image_file", fs.createReadStream(image.path)); // ✅ Key name for ClipDrop
...
export const reomveImageObject = async(req,res)=>{
    try{
        const {userId} = req.auth();
...
export const resumeReview = async(req,res)=>{
    try{
       ...
       const resume  = req.file;
        ...
        const prompt = `Review the following resume and provide  constructive feedback on its strength its  weakness and the  areas for  improvement  Resume Content:\n\n${pdfData.text}`
